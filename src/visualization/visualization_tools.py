...
def get_global_pac_linear(feats):
    pca = PCA(n_components=3)  # reduce dimensions to 3 for visualization
    _, _, _, c, _, _ = feats.shape  # torch.Size([1, 20, 1, 1024, 168, 252])
    feats = einops.rearrange(feats, 'b t v c h w -> (b t v h w) c')
    feats_pca = feats.to(torch.float16)
    feats_pca = pca.fit_transform(feats_pca)
    w = pca.components_
    mu = pca.mean_

    linear = torch.nn.Linear(in_features=c, out_features=pca.n_components_)
    linear.weight.data = w
    linear.bias.data = -mu @ w.T

    return linear.cuda()


def feat_visualizer(feat, pca_linear=None):
    v, _, h, w = feat.shape
    feat = einops.rearrange(feat, 'v c h w -> (v h w) c')
    if pca_linear is None:
        pca = PCA(n_components=3)  # reduce dimensions to 3 for visualization
        feat = pca.fit_transform(feat)
    else:
        feat = pca_linear(feat.to(pca_linear.weight.dtype))
    feat = (feat - feat.min()) / (feat.max() - feat.min())  # normalization
    feat = einops.rearrange(feat, '(v h w) c -> v c h w', v=v, h=h, w=w)
    return feat
...
def scene_flow_to_rgb(
    ...
    complex_flow = torch.complex(flow[..., 0], flow[..., 1])
    # radius, angle = op.abs(complex_flow), op.angle(complex_flow)
    radius, angle = torch.sqrt(complex_flow.real**2 + complex_flow.imag**2), op.angle(complex_flow)
