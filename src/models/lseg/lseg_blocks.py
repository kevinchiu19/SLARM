...
def _make_encoder(
    ...
    features,
    # use_pretrained=True,
    use_pretrained=False,  # avoid HTTP ERROR when downloading CLIP pretrained weights
    groups=1,
    expand=False,
    exportable=True,
    hooks=None,
...
class FeatureFusionBlock_custom(nn.Module):
    ...
    def forward(self, *xs):
        ...
        ori_dtype = output.dtype
        output = nn.functional.interpolate(
            output.to(torch.float), scale_factor=2, mode="bilinear", align_corners=self.align_corners
        )

        output = self.out_conv(output.to(ori_dtype))
