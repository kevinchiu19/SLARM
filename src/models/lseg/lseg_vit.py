...
def _resize_pos_embed(self, posemb, gs_h, gs_w):
    ...
    ori_dtype = posemb_grid.dtype  # aclnnUpsampleBilinear2d not implemented for DT_BFLOAT16
    posemb_grid = F.interpolate(posemb_grid.float().contiguous(), size=(gs_h, gs_w), mode="bilinear")
    posemb_grid = posemb_grid.to(ori_dtype)
...
def _make_vit_b16_backbone(
    ...
    pretrained.act_postprocess1 = nn.Sequential(
        readout_oper[0],
        nn.Linear(vit_features, features[0]),
        Transpose(1, 2),
        nn.ConvTranspose2d(
            in_channels=features[0],
            out_channels=features[0],
    ...
    pretrained.act_postprocess2 = nn.Sequential(
        readout_oper[1],
        nn.Linear(vit_features, features[1]),
        Transpose(1, 2),
        nn.ConvTranspose2d(
            in_channels=features[1],
            out_channels=features[1],
    ...
    pretrained.act_postprocess3 = nn.Sequential(
        readout_oper[2],
        nn.Linear(vit_features, features[2]),
        Transpose(1, 2),
    )
    ...
    pretrained.act_postprocess4 = nn.Sequential(
        readout_oper[3],
        nn.Linear(vit_features, features[3]),
        Transpose(1, 2),
        nn.Conv2d(
            in_channels=features[3],
            out_channels=features[3],
