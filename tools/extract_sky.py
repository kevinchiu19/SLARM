...
from src.dataset.constants import IMGNET_MEAN, IMGNET_STD
...
def get_args_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--rgb_type", type=str, default="jpg")
    parser.add_argument("--rgb_dir", type=str, default="images")
...
@torch.no_grad()
def get_sky_mask(dataloader: torch.utils.data.DataLoader, depthv2: DepthAnythingV2, rgb_type, rgb_dir) -> None:
    """
    ...
    for samples, paths in pbar:
        ...
        for i in range(len(sky_masks)):
            ...
            # e.g:
            tgt_pth = paths[i].replace(rgb_dir, "sky_masks")
            tgt_pth = tgt_pth.replace(rgb_type, "png")
...
def main(args: argparse.Namespace) -> None:
    ...
    get_sky_mask(data_loader, depthv2, args.rgb_type, args.rgb_dir)
