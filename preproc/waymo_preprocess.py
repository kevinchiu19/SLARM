...
# https://blog.csdn.net/weixin_41658139/article/details/126447734
# https://github.com/waymo-research/waymo-open-dataset/blob/master/tutorial/tutorial_2d_pvps.ipynb
def vis_panoptic(instance_label, semantic_label, path=f"panoptic_label_rgb.png"):
    ...
    instance_rgb = instance_label.astype(np.uint8)

    # Convert RGB to BGR for cv2 image saving
    panoptic_label_bgr = cv2.cvtColor(panoptic_label_rgb, cv2.COLOR_RGB2BGR)
    semantic_bgr = cv2.cvtColor(semantic_rgb, cv2.COLOR_RGB2BGR)

    # Assuming instance_rgb is a single-channel image, expand it to 3 channels for pseudo-color display
    if len(instance_rgb.shape) == 2 or instance_rgb.shape[2] != 3:
        instance_rgb = cv2.cvtColor(instance_rgb, cv2.COLOR_GRAY2BGR)
    instance_bgr = instance_rgb  # OpenCV reads as BGR by default, assuming already converted or inherently 3-channel

    # Create a blank vertically stacked image
    combined_image = np.vstack((panoptic_label_bgr, semantic_bgr, instance_bgr))
...
class WaymoProcessor:
    def __init__(
        ...
        num_workers=64,
        overwrite=False,
        only_process_semantic=False,
    ):
        self.overwrite = overwrite
        self.process_keys = process_keys
        self.only_process_semantic = only_process_semantic
        ...
        if self.only_process_semantic:
            print('Process only semantic data!')
    ...
    def convert_one(self, file_id):
        ...
        # num_frames = 2  # for debug

        if not self.only_process_semantic:
        ...
        # NOTE: waymo 1.4.3 contains semantic, but not lidar flow
        if "panoptic_segs" in self.process_keys:
            tfrecord_path = f"{self.load_dir}/{scene_name}.tfrecord"
            tfrecord_path = 'raw_1_4_3'.join(tfrecord_path.split('raw_scene_flow'))  # hardcode
            assert os.path.exists(tfrecord_path), 'The original tfrecord containing flow (waymo_flow) and  \
                                                    semantic (waymo_1_4_3) must be prepared at the same time!'
            dataset = tf.data.TFRecordDataset(tfrecord_path, compression_type="")
            num_frames = sum(1 for _ in dataset)
            for frame_id, data in enumerate(
                tqdm(dataset, desc=f"File {file_id}", total=num_frames, dynamic_ncols=True)
                ...
                frame = dataset_pb2.Frame()
                frame.ParseFromString(bytearray(data.numpy()))
                self.save_panoptic_segs(frame, file_id, frame_id)
        ...
        for frame_id in trange(num_frames, desc=f"Making video {scene_id}", dynamic_ncols=True):
            ...
            for cam_id in [3, 1, 0, 2, 4]:
                ...
                # NOTE: 886 // 4 = 221
                if img_array.shape[0] == 221:
                    img_array = img_array[:220, :, :]
                    depth_img = depth_img[:220, :, :]
                    flow_img = flow_img[:220, :, :]

                if img_array.shape[0] != 320:
                    _img_array = np.zeros((320, 480, 3))
                    _img_array[100:] = img_array
                    img_array = _img_array

                    _depth_img = np.zeros((320, 480, 3))
                    _depth_img[100:] = depth_img
                    depth_img = _depth_img

                    _flow_img = np.zeros((320, 480, 3))
                    _flow_img[100:] = flow_img
                    flow_img = _flow_img
    ...
    def save_panoptic_segs(self, frame, file_id, frame_id):
        """Parse and save the images in jpg format.
        ...
        if frame.images[0].camera_segmentation_label.panoptic_label:
            ...
            for i in range(len(segmentation_protos_ordered)):
                for camera in camera_order:
                    ...
                    semantic_label_path = (
                        f"{self.save_dir}/{str(scene_id).zfill(3)}/semantic_segs/"
                        + f"{str(frame_id).zfill(3)}_{str(cam_idx)}.npy"
                    )
                    instance_label_path = (
                        f"{self.save_dir}/{str(scene_id).zfill(3)}/instance_segs/"
                        + f"{str(frame_id).zfill(3)}_{str(cam_idx)}.npy"
                    )
                    if not os.path.exists(semantic_label_path) or self.overwrite:
                        np.save(semantic_label_path, semantic_label)
                    if not os.path.exists(instance_label_path) or self.overwrite:
                        np.save(instance_label_path, instance_label)
                        vis_panoptic(instance_label, semantic_label,
                                    f"{self.save_dir}/{str(scene_id).zfill(3)}/instance_segs/"
                                    + f"{str(frame_id).zfill(3)}_{str(cam_idx)}.png")
                    ...
                    for downsample_factor in self.downsample_factors:
                        if downsample_factor == 1:
                            ...
                            postfix = f"_{downsample_factor}"
                            downsampled_semantic_label_path = (
                                f"{self.save_dir}/{str(scene_id).zfill(3)}/semantic_segs{postfix}/"
                                + f"{str(frame_id).zfill(3)}_{str(cam_idx)}.npy"
                            )
                            if not os.path.exists(downsampled_semantic_label_path) or self.overwrite:
                                new_size = (
                                    semantic_label.shape[1] // downsample_factor,
                                    semantic_label.shape[0] // downsample_factor,
                                )
                                semantic_label_ds = cv2.resize(semantic_label, new_size, interpolation=cv2.INTER_NEAREST)
                                np.save(downsampled_semantic_label_path, semantic_label_ds)
                            downsampled_instance_label_path = (
                                f"{self.save_dir}/{str(scene_id).zfill(3)}/instance_segs{postfix}/"
                                + f"{str(frame_id).zfill(3)}_{str(cam_idx)}.npy"
                            )
                            if not os.path.exists(downsampled_instance_label_path) or self.overwrite:
                                new_size = (
                                    instance_label.shape[1] // downsample_factor,
                                    instance_label.shape[0] // downsample_factor,
                                )
                                instance_label_ds = cv2.resize(instance_label, new_size, interpolation=cv2.INTER_NEAREST)
                                np.save(downsampled_instance_label_path, instance_label_ds)
                                vis_panoptic(instance_label_ds, semantic_label_ds,
                                            f"{self.save_dir}/{str(scene_id).zfill(3)}/instance_segs{postfix}/"
                                            + f"{str(frame_id).zfill(3)}_{str(cam_idx)}.png")
    ...
    def create_folder(self):
        ...
        for i in self.scene_ids:
            ...
            for downsample_factor in self.downsample_factors:
                ...
                os.makedirs(f"{scene_path}/semantic_segs{postfix}", exist_ok=True)
                os.makedirs(f"{scene_path}/instance_segs{postfix}", exist_ok=True)
            ...
            os.makedirs(f"{scene_path}/semantic_segs", exist_ok=True)
            os.makedirs(f"{scene_path}/instance_segs", exist_ok=True)
